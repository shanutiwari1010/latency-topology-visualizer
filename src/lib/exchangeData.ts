...
export const createLatencyConnections = (
  ...
  return latencyData.map(data => {
    ...
    if (!source || !target) {
      throw new Error(`Exchange not found for connection ${data.id}`);
    }
...
export const filterExchangesByProvider = (
  ...
  return exchanges.filter(exchange => providers.includes(exchange.cloudProvider));
};
...
export const getExchangesByProvider = (provider: string): ExchangeLocation[] => {
  return EXCHANGE_LOCATIONS.filter(exchange => exchange.cloudProvider === provider);
};

export const calculateAverageLatency = (connections: LatencyConnection[]): number => {
